...
module Spree
  class Calculator::TaxWithShipping < Calculator
    ...
    def compute_order(order)
      ...
      line_items_total = matched_line_items.sum(&:total) + shipping_rate(order)
    ...
    def compute_shipment_or_line_item(item)
      if rate.included_in_price
        deduced_total_by_rate((item.pre_tax_amount + shipping_rate(item.order)), rate)
      else
        round_to_two_places((item.discounted_amount + shipping_rate(item.order)) * rate.amount)
      end
    end
